  ...
  render() {
    ...
    return (
      <StatusBarLayout>
        ...
        <p>Select how you would like to respond:</p>
        <div className="challenge-expired-button-container">
          <span className={css(styles.button)}>
            {challengeOptions.indexOf(ChallengeOptions.RespondWithMove) > -1 && (
              <Button onClick={selectRespondWithMove}>Respond with Move</Button>
            )}
            {challengeOptions.indexOf(ChallengeOptions.RespondWithExistingMove) > -1 && (
              <Button onClick={selectRespondWithExistingMove}>Respond with Existing Move</Button>
            )}
          </span>
        </div>
      </StatusBarLayout>
