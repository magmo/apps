...
import { responseProvided } from '../dispute/responder/actions';
  ...
  switch (action.type) {
    ...
    case 'WALLET.INDIRECT_DEFUNDING.UPDATE_CONFIRMED':
      ...
      if (protocolState.isRespondingToChallenge) {
        ...
        const responseProvidedAction = responseProvided({
          processId: sharedData.yieldingProcessId,
          commitment: ourCommitment,
        });

        newSharedData = helpers.yieldToProcess(
          queueInternalMessage(newSharedData, responseProvidedAction),
        );
      }
      if (!protocolState.isRespondingToChallenge) {
...
const waitForLedgerUpdateReducer = (
  ...
  switch (action.type) {
    ...
    case 'WALLET.INDIRECT_DEFUNDING.CHALLENGE_CHOSEN':
      const challengeAction = createChallengeRequested({ channelId: protocolState.ledgerId });
      newSharedData = queueInternalMessage(sharedData, challengeAction);
      return {
        protocolState: newProtocolState,
        sharedData: newSharedData,
      };
