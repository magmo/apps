 ...
 * @param iFrameId The id of the embedded wallet iframe.
 * @param signedState The data that was signed.
 *
 * @returns {Promise<void>} A promise that resolves when the state has been stored and validated.
 */
export async function validateAndSaveState(
  iFrameId: string,
  signedState: SignedState,
): Promise<boolean> {
  const iFrame = document.getElementById(iFrameId) as HTMLIFrameElement;
  const message = validateAndSaveRequest(signedState);
  ...
  const validatePromise = new Promise<boolean>((resolve, reject) => {
    window.addEventListener('message', function eventListener(event: MessageEvent) {
      if (
        event.data &&
        event.data.type &&
        (event.data.type === 'WALLET.STORE.SUCCESS' || event.data.type === 'WALLET.STORE.FAILURE')
      ) {
        const receivedMessage = event.data as ValidateAndSaveResponse;
        window.removeEventListener('message', eventListener);
        if (receivedMessage.type === 'WALLET.STORE.SUCCESS') {
          resolve();
        } else {
          const { error } = receivedMessage;
          reject({ error });
        }
      }
    });
 ...
 * @param iFrameId The id of the embedded wallet iframe.
 * @param state The state for the wallet to sign.
 * @returns {Promise<SignedState>} A promise that resolves to the signed state generated by the wallet.
 */
export async function signState(iFrameId: string, state: State): Promise<SignedState> {
  const iFrame = document.getElementById(iFrameId) as HTMLIFrameElement;
  const message = signStateRequest(state);

  const signPromise = new Promise<SignedState>((resolve, reject) => {
    window.addEventListener('message', function eventListener(event: MessageEvent) {
      if (
        event.data &&
        event.data.type &&
        (event.data.type === 'WALLET.SIGNATURE.SUCCESS' ||
          event.data.type === 'WALLET.SIGNATURE.FAILURE')
      ) {
        const receivedMessage = event.data as SignatureResponse;
        window.removeEventListener('message', eventListener);
        if (receivedMessage.type === 'WALLET.SIGNATURE.SUCCESS') {
          const { signedState } = receivedMessage;
          resolve(signedState);
        } else {
          const { error } = receivedMessage;
          reject({ error });
 ...
 */
export function respondToOngoingChallenge(iFrameId: string, responseState: SignedState) {
  const iFrame = document.getElementById(iFrameId) as HTMLIFrameElement;
  const message = respondToChallenge(responseState);
  iFrame.contentWindow.postMessage(message, '*');
}
